...
from app.core.config import GEMINI_MODEL, OUTPUT_UPLOADS_DIR
...
def analyze_requirement_changes(
    ...
    # LLM이 분석하기 용이하도록 기존 요구사항 리스트를 JSON 문자열로 변환
    req_list_str = json.dumps(existing_requirements, indent=2, ensure_ascii=False)

    prompt = f"""
    당신은 IT 프로젝트의 요구사항 변경을 관리하는 매우 꼼꼼한 시스템 분석가입니다.
    당신의 임무는 [기존 요구사항 목록]과 새로 들어온 [추가 자료(회의록)]를 비교 분석하여, 요구사항의 '추가', '수정', '삭제'가 필요한 항목을 식별하고 그 근거를 명확히 하여 JSON 형식으로 제안하는 것입니다.
    ...
    1.  **정확한 식별**: 회의록을 기반으로 변경이 필요한 기존 요구사항을 `requirement_id`를 통해 정확히 식별합니다.
    ...
    {{
      "to_add": [
        {{
          ...
          "importance": "HIGH",
          "difficulty": "MIDDLE",
          "modified_reason": "회의록에 따라 사용자 편의성 증대를 위해 추가"
        }}
      ],
      "to_update": [
        {{
          ...
          "importance": "HIGH",
          "difficulty": "LOW",
          "modified_reason": "회의록에 따라 인기순 정렬 옵션을 추가하기 위함"
        }}
      ],
      "to_delete": [
        {{
          "requirement_id": "USR-FR-0005",
          "modified_reason": "회의록의 '내 정보 조회/수정 기능 통합' 결정에 따라 해당 기능이 '내 정보 관리' 요구사항에 통합되었으므로 삭제 처리함."
        }}
      ]
    }}
    ```
